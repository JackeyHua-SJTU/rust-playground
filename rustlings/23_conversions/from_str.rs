...
impl FromStr for Person {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let name_parse = name.parse::<String>().unwrap();
        if name_parse.len() == 0 {
            return Err(Self::Err::NoName);
        }
        let age_parse = age.parse::<u8>().map_err(|x| Self::Err::ParseInt(x))?;
        
        Ok(Person{name: name_parse, age: age_parse})
    }
}
